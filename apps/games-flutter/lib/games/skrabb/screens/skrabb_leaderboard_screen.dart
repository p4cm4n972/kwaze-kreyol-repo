...
class SkrabbLeaderboardScreen extends StatefulWidget {
  const SkrabbLeaderboardScreen({super.key});

  @override
  State<SkrabbLeaderboardScreen> createState() =>
      _SkrabbLeaderboardScreenState();
}

class _SkrabbLeaderboardScreenState extends State<SkrabbLeaderboardScreen> {
  final SkrabbService _service = SkrabbService();

  bool _isLoading = true;
  SkrabbPlayerStats? _playerStats;
  List<SkrabbLeaderboardEntry> _leaderboard = [];
  String? _error;
  int? _playerRank;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Classement Skrabb'),
        backgroundColor: const Color(0xFFE74C3C), // Rouge madras
      ),
  ...
  Widget _buildStatItem(String label, String value, IconData icon) {
    return Column(
      children: [
        Icon(icon, size: 24, color: const Color(0xFFE74C3C)),
  ...
  Widget _buildLeaderboardItem(SkrabbLeaderboardEntry entry) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          'Temps: ${_formatTime(entry.timeElapsed)}',
  ...
  Color _getRankColor(int rank) {
    switch (rank) {
      ...
      default:
        return const Color(0xFFE74C3C); // Rouge madras
    }
  }
